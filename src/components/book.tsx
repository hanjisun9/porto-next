...
export default function Book() {
  useEffect(() => {
    ...
    PageTurnBtns.forEach((btn, index) => {
      btn.addEventListener("click", () => {
        ...
        const PageTurn = document.getElementById(
          PageTurnId!
    ...
    // opening animation (cover harus nutup full dulu)
    const coverRight = document.querySelector(
      ".cover.cover-right"
    ) as HTMLDivElement | null;
    const pageLeft = document.querySelector(
      ".book-page.page-left"
    ) as HTMLDivElement | null;
    ...
    if (coverRight) {
      // kasih z-index tinggi supaya nutup semua
      coverRight.style.zIndex = "999";

      // mulai animasi buka setelah delay
      setTimeout(() => {
        coverRight.classList.add("turn");
      }, 2100);

      // tunggu sampai animasi selesai
      coverRight.addEventListener("transitionend", () => {
        // cover pindah ke belakang
  ...
  return (
    <div className="book">
      ...
      <BookPage
        id="turn-2"
        front={<CerticatesFront />}
        back={<CertificatesBack />}
        side="right"
      />
      <BookPage
        id="turn-3"
        front={<ReflectionPage />}
        back={<SkillsPage />}
        side="right"
      />
      <BookPage
